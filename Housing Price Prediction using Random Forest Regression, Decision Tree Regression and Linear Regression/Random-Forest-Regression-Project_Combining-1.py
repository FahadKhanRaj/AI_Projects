...
def RFR(model, X, y):
    ...
    #map = 100 * np.mean(errors / y)
    #accuracy = 100 - map
    #print('Performance of the model: \n')
    #print('Average Error: {:0.4f} degrees'.format(np.mean(errors)))
    #print('Total Accuracy = {:0.8f} %'.format(accuracy))
...
RFR(reg,x_train,y_train)


# In[30]:


accuracyFR = RFR(reg,x_test,y_test)
print("Total accuracy is:",accuracyFR)
...
percent_difference_FR = (abs(y_test - y_pred_FR)/y_test) * 100        # for Forest Regression
for i in range(len(percent_difference_FR)):
  if percent_difference_FR[i] <= 1:
    percent_difference_FR[i] = 1
  else:
    percent_difference_FR[i] = 0
# print(percent_difference_FR)

pScoreFR = average_precision_score(percent_difference_y, percent_difference_FR)
rScoreFR = recall_score(percent_difference_y, percent_difference_FR)

print("Precision Score for Forest Regression is:",pScoreFR)
print("Recall Score for Forest Regression is:",rScoreFR)
...
CM_FR = confusion_matrix(percent_difference_y, percent_difference_FR)       # for Forest Regression
print("Confusion Matrix for Forest Regression is:\n",CM_FR,"\n")
